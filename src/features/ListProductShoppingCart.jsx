...
const ListProductShoppingCart = () => {
    ...
    const canIncreaseQty = (prod) => {
        const selectedQty = parseInt(prod.quantity_sold || 0, 10);
        const availableQty = parseInt(prod.quantity_product || 0, 10);
    ...
    const TotalPriceSupZero = (totalPriceBuy > 0)
    ...
    const boughtProduct = useCallback(async () => {
        ...
        try {
            await api.post("payments/cash/", payload, { headers: { 'Content-Type': 'application/json' } });
                    ...
                    <tbody>
                        {
                            itemsData?.map(prod => {
                                    ...
                                    const imageCurrentProdFirstVar=prod?.variants[0].image
                                        ...
                                        <tr key={prod?.id} className="border-gray-200 hover:bg-gray-50">
                                            ...
                                            <td className="p-1">
                                                <div className="w-10 h-10 md:w-32 md:h-32 overflow-hidden rounded-lg border border-gray-200 ">
                                                    <img src={imageCurrentProdFirstVar} alt={prod?.description_product || "Image du produit"} className="object-contain w-full h-full" loading="lazy" />
                                                </div>
                                            </td>
