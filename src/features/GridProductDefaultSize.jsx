...
const GridProductDefault = ({ data }) => {
    ...
    const [productData, setProductData] = useState(() =>
        ...
        Array.isArray(data) ? data.filter(p => p.quantity_product !== 0) : []
    );
    ...
    const addProductToCart = useCallback((product) => {
        ...
        dispatch(addMessageNotif(`Produit ${product?.code_reference} créé le ${Date.now()}`));
    ...
    const cols = useMemo(() => {
        ...
        return productData.filter(item => parseInt(item?.quantity_product) !== 0).reduce((acc, product, idx) => {
    ...
    useEffect(
        ...
        () => {
            ...
            if (Array.isArray(data)) {
                ...
                setProductData(data.filter(p => p.quantity_product !== 0));
            }
    ...
    useEffect(() => {
        ...
        const fetchProductsAndOwners = async () => {
            ...
            try {
                ...
                const ownerMap = responses.reduce(
